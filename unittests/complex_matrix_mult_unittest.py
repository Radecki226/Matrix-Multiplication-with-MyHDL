...
from myhdl import Signal,block,intbv,always,instances,ConcatSignal,delay,instance,traceSignals,StopSimulation,ResetSignal
...
import numpy as np
np.random.seed(2021)
#np.random.randint(MIN_INT,MAX_INT,K*M) + 1j*np.random.randint(MIN_INT,MAX_INT,K*M) 
COMPLEX_DAT_W = 16
MAX_INT = 32768
MIN_INT = -32768
ACCU_WIDTH = 80
...
class MatrixMultTest(unittest.TestCase):
    ...
    @block
    def tb(self,K,M,N,x_array,y_array,n_iter,rst_tick,rst_iter):
        #clk
        clk = Signal(bool(0))
        #rst
        ...
        def stimulus():
            ...
            for iter in range(n_iter):
                ...
                while(mm_done == 0 and rst == 0):
                    ...
                    if (processing_counter == rst_tick and iter == rst_iter):
                        rst.next = 1
                    else:
                        rst.next = 0
                mm_done_ack.next = 0
    ...
    def testWriteAndRead1(self):
        ...
        def test_mm1():
            ...
            rst_tick = None
            rst_iter = None
            tb = self.tb(K,M,N,x_array,y_array,n_iter,rst_tick,rst_iter)
            return instances()
